...
def Name(filename, fytime, plattype, outputpath, outTXTpath, prjType):
    ...
    if (len(shortName) == 8):
        ...
        scattertitle = "AHI8_IQUAM_SST %s" % (fytime.__format__('%Y-%m-%d %H:%M:%S'))
        histtitle = "AHI8_IQUAM_SST %s" % (fytime.__format__('%Y-%m-%d %H:%M:%S'))
        ...
        scattertitle = "AHI8_IQUAM(%s)_SST %s" % (platshortStr, fytime.__format__('%Y-%m-%d %H:%M:%S'))
        histtitle = "AHI8_IQUAM(%s)_SST %s" % (platshortStr, fytime.__format__('%Y-%m-%d %H:%M:%S'))
...
def Interp(tlat, tlon, slat, slon, sdata, dqf, spaceThreshold):
    ...
    slat[np.where(slat == 65534.)] = np.nan
...
def DrawScatter(mdat, param, totalNum, names):
    ...
    ax.set_xlabel('AHI8 SST(' + u'\u2103' + ')', fontsize=12)
