...
def DrawScatter(data1, data2, metric, filename, scattitle, checkVar, dataSour):
    ...
    ax.set_xlabel(dataSour + ' SST', fontsize=14)
    ax.set_ylabel("%s SST" % checkVar, fontsize=14)
...
def DrawHist(bias, metric, filename, histtitle, checkVar, dataSour):
    ...
    plt.xlim(-8, 8)
    ax.set_xlabel(dataSour + "-%s SST" % checkVar, fontsize=14)
...
def LogXML(logMsg, logType, nodes, dataSour, fyTime=True, metric=True, names=True):
    ...
    if (names != True):
        ...
        if metric['QualID3_AE'] > warnValue:
            ...
            nodewarnMsg.appendChild(doc.createTextNode("info" if not warntype else "精度预警：%s SST产品绝对偏差超过%s ℃" % (dataSour, warnValue)))
