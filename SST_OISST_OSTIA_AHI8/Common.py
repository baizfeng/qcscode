...
def Name(filename, fytime, outputpath, prjType, checkVar):
    ...
    scattertitle = "AHI8_%s_SST %s" % (checkVar, fytime.__format__('%Y-%m-%d %H:%M:%S'))
    histtitle = "AHI8-%s_SST %s" % (checkVar, fytime.__format__('%Y-%m-%d %H:%M:%S'))
    fy4title = "AHI8_SST_%s %s" % (prj, fytime.__format__('%Y-%m-%d %H:%M:%S'))
    biastitle = "AHI8-%s_SST_%s %s" % (checkVar, prj, fytime.__format__('%Y-%m-%d %H:%M:%S'))  # 标题
...
def Read_exterData(exterfilepath, prjType,cVar):
    ...
    if ((prjType == "GLL")&(cVar=="OSTIA")):
        ...
        fy4_mdata = ma.masked_values(fy4_data, -999.)
        fy4_mdata = fy4_mdata
    print fy4_mdata.shape
    print exterfilepath
    return fy4_mdata.data


def Calcate(fy4data, exdata, bias, dqf):
...
def DrawScatter(data1, data2, metric, filename, scattitle, checkVar):
    ...
    ax.set_xlabel('AHI8 SST'+'(' + u'\u2103' + ')', fontsize=14)
...
def DrawHist(bias, metric, filename, histtitle, checkVar):
    ...
    plt.xlim(-8, 8)
    ax.set_xlabel("AHI8-" + checkVar + " SST" + '(' + u'\u2103' + ')', fontsize=14)
...
# 绘制偏差分布图
def DrawGLLMap(data, lat, lon, titlename, filename, level=np.arange(-5, 36, 1)):
    fig = plt.figure(figsize=(36, 20), dpi=100)  # 图像的长*高=2748*3000
    axes1 = fig.add_axes([0., 0.1, 1., 0.9])  # 加两个panel，地图区和图例区域，四参数分别为：左边、下边框离边缘距离百分比，绘图区的宽和高
    ...
    m = Basemap(lon_0=0, resolution='l', ax=axes1)
    x, y = m(lon2d, lat2d)
    m.drawcoastlines()
    m.drawcountries()
...
def DrawNOMMap(data, lat, lon, titlename, filename, level=np.arange(-5, 36, 1)):
    ...
    m = Basemap(projection='nsper', lat_0=0, lon_0=140.7, resolution='l',
