...
def Name(filename, fytime, dataSour, outputpath, prjType, checkVar):
    if (prjType == 'NOM'):
        prj = "MAPN"
    elif (prjType == "GLL"):
        prj = "MAPG"

    scattername = "%s%s_QCS_%s_SCAT_ORIG.png" % (outputpath, filename, checkVar)
    histname = "%s%s_QCS_%s_HIST_BIAS.png" % (outputpath, filename, checkVar)
    fy4name = "%s%s_QCS_%s_ORIG.png" % (outputpath, filename, prj)
    checkname = "%s%s_QCS_%s_%s_ORIG.png" % (outputpath, filename, checkVar, prj)
    biasname = "%s%s_QCS_%s_%s_BIAS.png" % (outputpath, filename, checkVar, prj)
    scattertitle = "%s_%s_SST %s" % (dataSour, checkVar, fytime.__format__('%Y-%m-%d %H:%M:%S'))
    histtitle = "%s-%s_SST %s" % (dataSour, checkVar, fytime.__format__('%Y-%m-%d %H:%M:%S'))
    fy4title = "%s_SST_%s %s" % (dataSour, prj, fytime.__format__('%Y-%m-%d %H:%M:%S'))
    biastitle = "%s-%s_SST_%s %s" % (dataSour, checkVar, prj, fytime.__format__('%Y-%m-%d %H:%M:%S'))  # 标题
...
def TimeMatch(path, base_time, checkVar):
    """
    时间匹配：根据FY4时间得到检验数据
    :param path:
    :param base_time:
    ...
    if (checkVar == 'OISST'):
        files = glob.glob(os.path.join(exterPath, 'avhrr-only-v2.*.nc'))
    elif (checkVar == 'OSTIA'):
        files = glob.glob(os.path.join(exterPath, '*UKMO-L4_GHRSST-SSTfnd-OSTIA-GLOB-v02.0-fv02.0.nc'))
    base_time2str = base_time.__format__('%Y%m%d')

    for filepath in files:
        file = os.path.basename(filepath)
        if (checkVar == 'OISST'):
            timeStr = file.split('.')[1][:8]
        elif (checkVar == 'OSTIA'):
            timeStr = file.split('-')[0][:-6]
...
def Read_exterData(exterfilepath, prjType,cVar):
    """
    读取检验源数据信息
    ...
    with Dataset(exterfilepath, 'r') as fid:
        fy4_data = fid.variables[var][:]
    if (isinstance(fy4_data, ma.MaskedArray)):
        fy4_data = fy4_data.data
    if ((prjType == "GLL")&(cVar=="OSTIA")):
        fy4_mdata = ma.masked_values(fy4_data, -999.)
        fy4_mdata = fy4_mdata - 273.15
    else:
        fy4_mdata = ma.masked_values(fy4_data, -999.)
        fy4_mdata = fy4_mdata
    return fy4_mdata.data
...
def Calcate(fy4data, exdata, bias):
    fy4data3 = fy4data[~fy4data.mask]
    exdata3 = exdata[~exdata.mask]
    bias3 = bias[~bias.mask]
    # 指标计算
...
# 绘制散点图
def DrawScatter(data1, data2, metric, filename, scattitle, checkVar,dataSour):
    ...
    ax.set_xlabel(dataSour+' SST'+'(' + u'\u2103' + ')', fontsize=14)
...
def DrawHist(bias, metric, filename, histtitle, checkVar,dataSour):
    # 创建直方图
    # 第一个参数为待绘制的定量数据
    # 第二个参数为划分的区间个数
    bias = bias[~bias.mask]
    ...
    # # add a 'best fit' line
    # y = mlab.normpdf(bins, metric['QualID0_MEAN'], metric['QualID0_STD'])
    ...
    plt.xlim(-8, 8)
    ax.set_xlabel(dataSour+"-"+checkVar+" SST"+'(' + u'\u2103' + ')', fontsize=14)
...
def DrawNOMMap(data, lat, lon, dataSour, titlename, filename, level=np.arange(-5, 36, 1)):
    if (dataSour == 'FY2F'):
        lon0 = 112.
    elif (dataSour == 'FY2G'):
        lon0 = 104.5
    ...
    m = Basemap(projection='nsper', lat_0=0, lon_0=lon0, resolution='l',
...
def LogXML(logMsg, logType, nodes,dataSour, fyTime=True, metric=True, names=True):
    ...
    if (names != True):
        ...
        nodeDeviationstatic.setAttribute('productVariety', dataSour+'#SST')
        metricStr = "{%s}" % re.sub('[()\'\[\]]', '',
