...
def TimeMatch(path, base_time, timeThreshold):
    ...
    :return:
    """
    files = glob.glob(os.path.join(path, 'AHI8-_AGRI*N_DISK*FHS*NOM*V0001.NC'))  # AHI8-_AGRI*N_DISK*FHS*NOM*V0001.NC
    dfiles = []
    dtimes = []
...
def Name(filename, fy4time, ahi8times, outputpath):
    """
    定义输出的文件名或文件标题
    :param baseInfo:
    :param nodes:
    ...
    :return:
    """
    histname = "%s%s_QCS_AHI8_HIST_BIAS.png" % (outputpath, filename)
    histtitle = u"FY4A时间:%s UTC AHI8时间:%s UTC" % (
        fy4time.__format__('%Y-%m-%d %H:%M:%S'), ahi8times[0].__format__('%Y-%m-%d %H:%M:%S'))
    biastitle = u"FY4A-AHI8火点像元位置匹配示意图\n FY4A时间:%s UTC  AHI8时间:%s UTC" % (
        fy4time.__format__('%Y-%m-%d %H:%M:%S'), ahi8times[0].__format__('%Y-%m-%d %H:%M:%S'))
    BIASMap = "%s%s_QCS_AHI8_MAPN_BIAS.png" % (outputpath, filename)
...
def Read_cLUT():
    ...
    :return:
    """
    lutpath = r'/FY4APGSQCS/QCS/pyFY4AL2src/LUT/AHI8_OBI_2000M_NOM_LATLON.HDF'
...
def Interp(tlat, tlon, slat, slon, sdata, spaceThreshold):
    ...
    slat[np.where(slat == -9999)] = np.nan  # 注意小数点
    slon[np.where(slon == -9999)] = np.nan
    ...
    d, inds = tree.query(zip(tlon.flatten(), tlat.flatten()), k=1, distance_upper_bound=float(spaceThreshold))
...
def MatchFY4_AHI8(fy4_mdata, ahi8_mdata, Reli):
    """
    FY4与AHI8空间匹配，若FY4火点7*7范围内有AHI8的火点，则表示该火点匹配成功，匹配上的点为3，返回二者匹配的行列号和匹配结果
    :param fy4_mdata:
    :param ahi8_mdata:
    :return:
    """
    FY4 = np.array(np.where(fy4_mdata == 10)).T
    AHI8 = np.array(np.where(ahi8_mdata == 10)).T
    ahi8_Line = AHI8[:, 0]
    ahi8_Column = AHI8[:, 1]
    m1 = FY4.shape[0]
    m2 = AHI8.shape[0]
...
def DrawHist(statisRes, histname, histtitle):
    ...
    font = FontProperties(fname=r"/FY4APGSQCS/QCS/src/FHS/FHS_FY3B_FY4A/Logo/simhei.ttf")
    ...
    fig.text(0.05, 0.9, u"FY4A-AHI8火点匹配准确率", fontsize=18, fontproperties=font)
...
def DrawBIASMap(data, lat, lon, titlename, filename):
    ...
    fig = plt.figure(figsize=(54.96, 60), dpi=100)  # 图像的长*高=2748*3000
    ...
    m.scatter(x[mbias == 1], y[mbias == 1], s=140, color="red")
    m.scatter(x[mbias == 2], y[mbias == 2], s=140, color="blue")
    m.scatter(x[mbias == 3], y[mbias == 3], s=140, color="yellow")
    ...
    names = [u"FY4A未匹配像元", u"AHI8未匹配像元", u"匹配像元"]
    for i in range(len(colors)):
        xx = 0.1 + 0.15 * i
        yy = 0.22
        axes2.add_patch(Ellipse(xy=(xx, yy), width=0.015, height=0.15, color=colors[i]))
        axes2.text(xx + 0.01, yy - 0.04, names[i], fontsize=70,fontproperties=font)
    #添加Logo
...
def LogXML(logMsg, logType, xmlfilePath, inputFilename, fyTime=True, metric=True, names=True):
    ...
    if (names != True):
        ...
        nodeOutputFilename3.appendChild(doc.createTextNode(str(os.path.basename(names['BIASMap']))))
        ...
        nodeDeviationstatic.setAttribute('checkVariety', "AHI8")
