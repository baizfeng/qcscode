...
def Name(filename, fytime, plattype, outputpath, outTXTpath, prjType):
    ...
    if (len(shortName) == 8):
        ...
        # 带标签中间文件
        txtnamelabel = "%s%s00000000%s_QCS_IQUAM(FULL)_DATA_BIAS_NA.txt" % (outTXTpath, filename[:50], filename[73:])
        txtname = "%s%s00000000%s_QCS_IQUAM_DATA_BIAS_NA.txt" % (outTXTpath, filename[:50], filename[73:])  # 全类型中间文件
    else:
        ...
        # 带标签中间文件
        txtnamelabel = "%s%s00000000%s_QCS_IQUAM-%s_DATA_BIAS_NA.txt" % (
