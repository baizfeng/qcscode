...
def Name(filename, fytime, plattype, outputpath, dataSour, isna):
    if (isna == 'false'):
        shortName = Plattypeprocess(plattype)
        if (len(shortName) == 8):
            histname = "%s%s000000_AOAD_QCS_IQUAM_HIST_BIAS.png" % (outputpath, filename[:25])
            scattername = "%s%s000000_AOAD_QCS_IQUAM_SCAT_ORIG.png" % (outputpath, filename[:25])
            fy4name = "%s%s000000_AOAD_QCS_MAPN_ORIG.png" % (outputpath, filename[:25])
            histtitle = "%s_IQUAM_SST %s" % (dataSour, fytime.__format__('%Y-%m-%d'))
            scattertitle = "%s_IQUAM_SST %s" % (dataSour, fytime.__format__('%Y-%m-%d'))
            fy4title = "%s_SST_MAPN %s" % (dataSour, fytime.__format__('%Y-%m-%d'))  # 标题
            iquamtitle = "%s_IQUAM_SST %s" % (dataSour, fytime.__format__('%Y-%m-%d'))
            iquamname = "%s%s000000_AOAD_QCS_IQUAM_MAPG_ORIG.png" % (outputpath, filename[:25])
        else:
            platshortStr = '-'.join(shortName)
            histname = "%s%s000000_AOAD_QCS_IQUAM-%s_HIST_BIAS.png" % (outputpath, filename[:25], platshortStr)
            scattername = "%s%s000000_AOAD_QCS_IQUAM-%s_SCAT_ORIG.png" % (outputpath, filename[:25], platshortStr)
            fy4name = "%s%s000000_AOAD_QCS_MAPN_ORIG.png" % (outputpath, filename[:25])
            scattertitle = "%s_IQUAM(%s)_SST %s" % (dataSour, platshortStr, fytime.__format__('%Y-%m-%d'))
            histtitle = "%s_IQUAM(%s)_SST %s" % (dataSour, platshortStr, fytime.__format__('%Y-%m-%d'))
            fy4title = "%s_SST_MAPN %s" % (dataSour, fytime.__format__('%Y-%m-%d'))  # 标题
            iquamtitle = "%s_IQUAM(%s)_SST %s" % (dataSour, platshortStr, fytime.__format__('%Y-%m-%d'))
            iquamname = "%s%s000000_AOAD_QCS_IQUAM-%s_MAPG_ORIG.png" % (outputpath, filename[:25], platshortStr)
    elif (isna == 'true'):
        shortName = Plattypeprocess(plattype)
        if (len(shortName) == 8):
            histname = "%s%s000000_AOAD_QCS_IQUAM_HIST_BIAS_NA.png" % (outputpath, filename[:25])
            scattername = "%s%s000000_AOAD_QCS_IQUAM_SCAT_ORIG_NA.png" % (outputpath, filename[:25])
            fy4name = "%s%s000000_AOAD_QCS_MAPN_ORIG_NA.png" % (outputpath, filename[:25])
            histtitle = "%s_IQUAM_SST %s" % (dataSour, fytime.__format__('%Y-%m-%d'))
            scattertitle = "%s_IQUAM_SST %s" % (dataSour, fytime.__format__('%Y-%m-%d'))
            fy4title = "%s_SST_MAPN %s" % (dataSour, fytime.__format__('%Y-%m-%d'))  # 标题
            iquamtitle = "%s_IQUAM_SST %s" % (dataSour, fytime.__format__('%Y-%m-%d'))
            iquamname = "%s%s000000_AOAD_QCS_IQUAM_MAPG_ORIG_NA.png" % (outputpath, filename[:25])
        else:
            platshortStr = '-'.join(shortName)
            histname = "%s%s000000_AOAD_QCS_IQUAM-%s_HIST_BIAS_NA.png" % (outputpath, filename[:25], platshortStr)
            scattername = "%s%s000000_AOAD_QCS_IQUAM-%s_SCAT_ORIG_NA.png" % (outputpath, filename[:25], platshortStr)
            fy4name = "%s%s000000_AOAD_QCS_MAPN_ORIG_NA.png" % (outputpath, filename[:25])
            scattertitle = "%s_IQUAM(%s)_SST %s" % (dataSour, platshortStr, fytime.__format__('%Y-%m-%d'))
            histtitle = "%s_IQUAM(%s)_SST %s" % (dataSour, platshortStr, fytime.__format__('%Y-%m-%d'))
            fy4title = "%s_SST_MAPN %s" % (dataSour, fytime.__format__('%Y-%m-%d'))  # 标题
            iquamtitle = "%s_IQUAM(%s)_SST %s" % (dataSour, platshortStr, fytime.__format__('%Y-%m-%d'))
            iquamname = "%s%s000000_AOAD_QCS_IQUAM-%s_MAPG_ORIG_NA.png" % (outputpath, filename[:25], platshortStr)
    names = {'histname': histname,
             'fy4name': fy4name,
             'scatname': scattername,
             'histtitle': histtitle,
...
def MatchTXT(fy4time, nodes):
    ...
    if (isna == 'false'):
        ...
        elif (len(shortName) == 2):
            pattern = ['QCS_IQUAM-' + shortName[0] + '-' + shortName[1] + '_DATA_BIAS.txt', nodes['type'],
                       fy4time.__format__('%Y%m')]
        else:
            pattern = ['QCS_IQUAM-' + shortName[0] + '_DATA_BIAS.txt', nodes['type'], fy4time.__format__('%Y%m')]
    elif (isna == 'true'):
        shortName = Plattypeprocess(nodes['platformtype'])
        if (len(shortName) == 8):
            pattern = ['QCS_IQUAM_DATA_BIAS_NA.txt', nodes['type'], fy4time.__format__('%Y%m')]
        elif (len(shortName) == 2):
            pattern = ['QCS_IQUAM-' + shortName[0] + '-' + shortName[1] + '_DATA_BIAS_NA.txt', nodes['type'],
                       fy4time.__format__('%Y%m')]
        else:
            pattern = ['QCS_IQUAM-' + shortName[0] + '_DATA_BIAS_NA.txt', nodes['type'], fy4time.__format__('%Y%m')]
    result = []  # 所有的文件
...
def ReadTXTDataH(filepath, fy4time):
    timestr = fy4time.__format__('%Y%m%d')
...
def ReadTXTDataG(filepath, fy4time):
    timestr = fy4time.__format__('%Y%m%d')
    with open(filepath, 'r') as r:
        lines = r.readlines()
...
def Read_Data(filepath, dataSour):
    ...
    with Dataset(filepath, 'r') as fid:
        fy4_data = fid.variables['%s SST Daily Mean Product' % dataSour][:]
...
def DrawScatterH(mdat, param, totalNum, names):
    ...
    H0 = ax.scatter(fySST[np.where(fyDQF == 0)], exSST[np.where(fyDQF == 0)], c='r', marker='o', s=5)
    H1 = ax.scatter(fySST[np.where(fyDQF == 1)], exSST[np.where(fyDQF == 1)], c='g', marker='D', s=5)
    H2 = ax.scatter(fySST[np.where(fyDQF == 2)], exSST[np.where(fyDQF == 2)], c='b', marker='x', s=5)
