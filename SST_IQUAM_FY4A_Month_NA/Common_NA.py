...
def Calcate(mdat, na):
    ...
    bias00 = bias0[np.abs(bias0) < float(na)]
    bias10 = bias1[np.abs(bias1) < float(na)]
    bias20 = bias2[np.abs(bias2) < float(na)]
    bias30 = bias3[np.abs(bias3) < float(na)]

    if (bias00.shape[0] != 0):
        slope0, intercept0, r_value0, p_value0, std_err0 = stats.linregress(
            fySST[np.where(fyDQF == 0)][np.abs(bias0) < float(na)],
            exSST[np.where(fyDQF == 0)][np.abs(bias0) < float(na)])
    else:
    ...
    if (bias10.shape[0] != 0):
        slope1, intercept1, r_value1, p_value1, std_err1 = stats.linregress(
            fySST[np.where(fyDQF == 1)][np.abs(bias1) < float(na)],
            exSST[np.where(fyDQF == 1)][np.abs(bias1) < float(na)])
    else:
        slope1, intercept1, r_value1, p_value1, std_err1 = [np.nan, np.nan, np.nan, np.nan, np.nan]
    if (bias20.shape[0] != 0):
        slope2, intercept2, r_value2, p_value2, std_err2 = stats.linregress(
            fySST[np.where(fyDQF == 2)][np.abs(bias2) < float(na)],
            exSST[np.where(fyDQF == 2)][np.abs(bias2) < float(na)])
    else:
    ...
    if (bias30.shape[0] != 0):
        slope3, intercept3, r_value3, p_value3, std_err3 = stats.linregress(
            fySST[np.where(fySST != -999)][np.abs(bias3) < float(na)],
            exSST[np.where(fySST != -999)][np.abs(bias3) < float(na)])
    else:
        slope3, intercept3, r_value3, p_value3, std_err3 = [np.nan, np.nan, np.nan, np.nan, np.nan]

    num0 = bias00.shape[0]  # 样本数量
    mean0 = round(np.mean(bias00), 4)  # 平均偏差
    absmean0 = round(np.mean(np.abs(bias00)), 4)  # AE绝对平均偏差
    std0 = round(np.sqrt(np.square(bias00).sum() / (num0-1)), 4)  # 标准差
    max0 = round(np.max(bias00), 4) if num0 != 0 else np.nan  # 最大值
    min0 = round(np.min(bias00), 4) if num0 != 0 else np.nan  # 最小值
    median0 = round(np.median(bias00), 4)  # 中位数
    rmse0 = round(np.sqrt(np.square(bias00).sum() / num0), 4)  # 均方根误差
    skew0 = round(stats.skew(bias00), 4)  # 偏度系数
    kurt0 = round(stats.kurtosis(bias00), 4)  # 峰度系数

    num1 = bias10.shape[0]  # 样本数量
    mean1 = round(np.mean(bias10), 4)  # 平均偏差
    absmean1 = round(np.mean(np.abs(bias10)), 4)  # AE绝对平均偏差
    std1 = round(np.sqrt(np.square(bias10).sum() / (num1-1)), 4)  # 标准差
    max1 = round(np.max(bias10), 4) if num1 != 0 else np.nan  # 最大值
    min1 = round(np.min(bias10), 4) if num1 != 0 else np.nan  # 最小值
    median1 = round(np.median(bias10), 4)  # 中位数
    rmse1 = round(np.sqrt(np.square(bias10).sum() / num1), 4)  # 均方根误差
    skew1 = round(stats.skew(bias10), 4)  # 偏度系数
    kurt1 = round(stats.kurtosis(bias10), 4)  # 峰度系数

    num2 = bias20.shape[0]  # 样本数量
    mean2 = round(np.mean(bias20), 4)  # 平均偏差
    absmean2 = round(np.mean(np.abs(bias20)), 4)  # AE绝对平均偏差
    std2 = round(np.sqrt(np.square(bias20).sum() / (num2-1)), 4)  # 标准差
    max2 = round(np.max(bias20), 4) if num2 != 0 else np.nan  # 最大值
    min2 = round(np.min(bias20), 4) if num2 != 0 else np.nan  # 最小值
    median2 = round(np.median(bias20), 4)  # 中位数
    rmse2 = round(np.sqrt(np.square(bias20).sum() / num2), 4)  # 均方根误差
    skew2 = round(stats.skew(bias20), 4)  # 偏度系数
    kurt2 = round(stats.kurtosis(bias20), 4)  # 峰度系数

    num3 = bias30.shape[0]  # 样本数量
    mean3 = round(np.mean(bias30), 4)  # 平均偏差
    absmean3 = round(np.mean(np.abs(bias30)), 4)  # AE绝对平均偏差
    std3 = round(np.sqrt(np.square(bias30).sum() / (num3-1)), 4)  # 标准差
    max3 = round(np.max(bias30), 4) if num3 != 0 else np.nan  # 最大值
    min3 = round(np.min(bias30), 4) if num3 != 0 else np.nan  # 最小值
    median3 = round(np.median(bias30), 4)  # 中位数
    rmse3 = round(np.sqrt(np.square(bias30).sum() / num3), 4)  # 均方根误差
    skew3 = round(stats.skew(bias30), 4)  # 偏度系数
    kurt3 = round(stats.kurtosis(bias30), 4)  # 峰度系数
...
def DrawScatter(mdat, param, totalNum, names, na):
    exSST = mdat['exSST']
    fySST = mdat['fySST']
    fyDQF = mdat['fyDQF']
    ...
    H0 = ax.scatter(fySST[np.where(fyDQF == 0)][np.abs(bias0) < float(na)],
                    exSST[np.where(fyDQF == 0)][np.abs(bias0) < float(na)], c='r', marker='o')
    H1 = ax.scatter(fySST[np.where(fyDQF == 1)][np.abs(bias1) < float(na)],
                    exSST[np.where(fyDQF == 1)][np.abs(bias1) < float(na)], c='g', marker='D')
    H2 = ax.scatter(fySST[np.where(fyDQF == 2)][np.abs(bias2) < float(na)],
                    exSST[np.where(fyDQF == 2)][np.abs(bias2) < float(na)], c='b', marker='x')
...
def DrawHist(bias, metric, names,na):
    # 创建直方图
    # 第一个参数为待绘制的定量数据
    # 第二个参数为划分的区间个数
    bias = bias[np.abs(bias) < float(na)]
