...
def Name(filename, fytime, plattype, outputpath, outTXTpath, prjType, dataSour):
    ...
    if (len(shortName) == 8):
        ...
        # 带标签中间文件
        txtnamelabel = "%s%s_QCS_IQUAM(FULL)_DATA_BIAS.txt" % (outTXTpath, filename[:-7])
        txtname = "%s%s_QCS_IQUAM_DATA_BIAS.txt" % (outTXTpath, filename[:-7])  # 全类型中间文件
    else:
        ...
        # 带标签中间文件
        txtnamelabel = "%s%s_QCS_IQUAM-%s_DATA_BIAS.txt" % (outTXTpath, filename[:-7], platshortStr)
        txtname = "%s%s_QCS_IQUAM_DATA_BIAS.txt" % (outTXTpath, filename[:-7])  # 全类型中间文件
...
def DrawScatterH(mdat, param, totalNum, names):
    ...
    ax.set_xlabel('FY2H SST', fontsize=12)
...
def DrawScatterG(mdat, param, totalNum, names, dataSour):
    exSST = mdat['exSST']
    fySST = mdat['fySST']
    ...
    ax.set_xlabel(dataSour + ' SST', fontsize=12)
    ax.set_ylabel('IQUAM SST', fontsize=12)
