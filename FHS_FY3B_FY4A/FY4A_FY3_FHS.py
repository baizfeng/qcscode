...
if __name__ == "__main__":
    try:
        ...
        print baseInfo
        fy3files, fy3times = TimeMatch(nodes['exterpath'], baseInfo['time'], nodes['timevalue'], nodes['checkVar'])
        QCSlog("时间匹配完成:" + str(fy3files), nodes['xmlPath'])

        # 输出的文件名和标题
        names = Name(baseInfo, nodes, fy3times, nodes['checkVar'])
        ...
        QCSlog("FY3 火点数据读取完成", nodes['xmlPath'])
        ...
        # 输出统计结果直方图
        metric = DrawHist(fy4dat_check, trueNum, names['histname'], names['histtitle'], nodes['checkVar'])
        ...
        pixelCondition, rect = PixelConditionMap(MatchPixel_reject, baseInfo['time'], fy3times[0],
                                                 names['pixelCondMap'], nodes['checkVar'])
        ...
        # 输出日志xml
        LogXML(pixelSketch, "info", nodes['xmlPath'], nodes['filepath'], nodes['checkVar'], baseInfo['time'], metric,
               names)
