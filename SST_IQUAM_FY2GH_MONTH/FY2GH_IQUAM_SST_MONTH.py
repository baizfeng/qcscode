#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/11/7 11:23
# @Author  : baizhaofeng
# @File    : FY2GH_IQUAM_SST_MONTH.py
...
if __name__ == "__main__":
    try:
        ...
        # 输出的文件名和标题
        names = Name(baseInfo['filename'], baseInfo['time'], nodes['outputpath'],baseInfo['dataSour'],nodes['isna'])
        txtfile = MatchTXT(baseInfo['time'], nodes)

        # 读取中匹配的中间文件
        if (baseInfo['dataSour'] == 'FY2H'):
            mdat = ReadTXTDataH(txtfile)
        elif (baseInfo['dataSour'] == 'FY2G'):
            mdat = ReadTXTDataG(txtfile)
        # 3.读取数据
