...
def TimeMatch(path, base_time, timeThreshold):
    ...
    :return:
    """
    path = path + base_time.__format__('%Y-%m-%d')
    files = glob.glob(os.path.join(path, '*PLSF*FY3D*.txt'))
    dfiles = []
    dtimes = []
    for filepath in files:
        file = os.path.basename(filepath)
        timeStr = file.split('_')[6][:-4]
...
def Name(baseInfo, nodes, fy3times):
    """
    定义输出的文件名或文件标题
    :param baseInfo:
    :param nodes:
    ...
    txtpath = "%s%s_QCS_FY3D_DATA_MATCH.TXT" % (outputpath, filename)
    ncpath = "%s%s_QCS_FY3D_DATA_BIAS.NC" % (outputpath, filename)
    histname = "%s%s_QCS_FY3D_HIST_BIAS.png" % (outputpath, filename)
    histtitle = u"FY4A时间:%s UTC FY3D时间:%s UTC" % (
        fy4time.__format__('%Y-%m-%d %H:%M:%S'), fy3times[0].__format__('%Y-%m-%d %H:%M:%S'))
    pixelCondMap = "%s%s_QCS_FY3D_MAPN_PIXEL_CONDITION.png" % (outputpath, filename)
    areaSketchMap = "%s%s_QCS_FY3D_MAPN_AREA_SKETCH.png" % (outputpath, filename)
    pixelSketchMap = "%s%s_QCS_FY3D_MAPN_PIXEL_SKETCH.png" % (outputpath, filename)
...
def DrawHist(fy4dat_check, trueNum, histname, histtitle):
    ...
    font = FontProperties(fname="/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/simhei.ttf")
    ...
    fig.text(0.05, 0.9, u"FY4A-FY3D火点匹配准确率", fontsize=18, fontproperties=font)
...
def PixelConditionMap(fire, fy4time, fy3time, output):
    font = FontProperties(fname="/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/simhei.ttf")
    ...
    m.readshapefile('/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/country', 'country')
    m.readshapefile('/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/provice', 'provice')
    m2.readshapefile('/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/country', 'country')
    m2.readshapefile('/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/provice', 'provice')
    patches = []
    patches2 = []
    ...
    axes1.set_title(u"FY4A-FY3D火点像元位置匹配实况图", fontproperties=font, fontsize=120)
    ...
    axicon.imshow(plt.imread('/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/logo.png'), origin='upper')
    axicon.axis('off')
    # 添加时间
    axtime = fig.add_axes([0.007, 0.368, 0.135, 0.023])
    axtime.text(0.04, 0.1, "FY4A Time:%s\nFY3D Time:%s" % (
        fy4time.__format__('%Y-%m-%d %H:%M'), fy3time.__format__('%Y-%m-%d %H:%M')), fontsize=64)
    axtime.set_xticks([])
    axtime.set_yticks([])
    # 添加图例
    axlegend = fig.add_axes([0.005, 0.005, 0.07, 0.05])
    axlegend.imshow(plt.imread('/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/Lengend-PixelCondition.png'), origin='upper')
    axlegend.axis('off')
    # 保存图片
...
def AreaSketchMap(firezone, fy4time, fy3time, output, rect):
    font = FontProperties(fname="/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/simhei.ttf")
    ...
    axes1.add_collection(PatchCollection(patches, facecolor="#bebebe"))
    axes2.add_collection(PatchCollection(patches2, facecolor="#bebebe"))
    axes1.set_title(u"FY4A-FY3D火区位置匹配示意图", fontproperties=font, fontsize=120)
    ...
    # 添加图例
    axlegend = fig.add_axes([0.005, 0.005, 0.07, 0.05])
    axlegend.imshow(plt.imread('/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/Lengend-AreaSketch.png'), origin='upper')
    axlegend.axis('off')
    # 保存图片
    fig.savefig(output, bbox_inches='tight')
    plt.close()
...
def PixelSketchMap(firepixel, fy4time, fy3time, output, rect):
    font = FontProperties(fname="/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/simhei.ttf")
    xmin, xmax, ymin, ymax = rect
    ...
    axes1.add_collection(PatchCollection(patches, facecolor="#bebebe"))
    axes2.add_collection(PatchCollection(patches2, facecolor="#bebebe"))
    axes1.set_title(u"FY4A-FY3D火点像元位置匹配示意图", fontproperties=font, fontsize=120)
    ...
    # 添加图例
    axlegend = fig.add_axes([0.005, 0.005, 0.07, 0.05])
    axlegend.imshow(plt.imread('/FY4APGSQCS/QCS/src/FHS/FHS_FY3D_FY4A/Logo/Lengend-PixelSketch.png'), origin='upper')
    axlegend.axis('off')
    # 保存图片
    fig.savefig(output, bbox_inches='tight')
    plt.close()
...
def LogXML(logMsg, logType, xmlfilePath, inputFilename, fyTime=True, metric=True, names=True):
    ...
    if (names != True):
        ...
        nodeDeviationstatic.setAttribute('checkVariety', "FY3D")
