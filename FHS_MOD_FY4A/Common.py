...
def TimeMatch(path, base_time, timeThreshold):
    ...
    files = glob.glob(os.path.join(path, '*.hdf'))  # AHI8-_AGRI*N_DISK*FHS*NOM*V0001.NC
    dfiles = []
    dtimes = []
...
def Name(filename, fy4time, ahi8time, checkVar, outputpath):
    """
    定义输出的文件名或文件标题
    :param baseInfo:
    :param nodes:
    ...
    histtitle = u"FY4A时间:%s UTC %s时间:%s--%s UTC" % (
        fy4time.__format__('%Y-%m-%d %H:%M:%S'), checkVar, ahi8time[0].__format__('%Y-%m-%d %H:%M:%S'),
        ahi8time[len(ahi8time) - 1].__format__('%Y-%m-%d %H:%M:%S'))
    biastitle = u"FY4A-%s火点像元位置匹配示意图\n FY4A时间:%s UTC  %s时间:%s--%s UTC" % (checkVar,
                                                                          fy4time.__format__('%Y-%m-%d %H:%M:%S'),
                                                                          checkVar,
                                                                          ahi8time[0].__format__('%Y-%m-%d %H:%M:%S'),
                                                                          ahi8time[len(ahi8time) - 1].__format__(
                                                                              '%H:%M:%S'))
    BIASMap = "%s%s_QCS_%s_MAPN_BIAS.png" % (outputpath, filename, checkVar)
...
def ReadExterData(filespath):
    ...
    for filepath in filespath:
        fidmod = SD(filepath.encode('utf-8'), SDC.READ)
        lat_sds = fidmod.select('FP_latitude')
        lon_sds = fidmod.select('FP_longitude')
        if lat_sds.dimensions()['number_of_active_fires']:
            lat = lat_sds.get()
            lon = lon_sds.get()
            if lat.shape[0]:
                if lats is None:
...
def DrawHist(statisRes, histname, histtitle,checkVar):
    trueNum = statisRes[1]
    totalNum = statisRes[0]
    acc = np.flipud(trueNum * 1.0 / totalNum)

    # plt.rcParams['font.family'] = ['SimHei']  # 用来正常显示中文标签
    font = FontProperties(fname=r"/FY4APGSQCS/QCS/src/FHS/FHS_MOD_FY4A/Logo/simhei.ttf")
...
def DrawBIASMap(data, lat, lon, titlename, filename,checkVar):
    font = FontProperties(fname=r"/FY4APGSQCS/QCS/src/FHS/FHS_MOD_FY4A/Logo/simhei.ttf")
    ...
    names = [u"FY4A未匹配像元", u"%s未匹配像元"%checkVar, u"匹配像元"]
