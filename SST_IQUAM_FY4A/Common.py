...
import seaborn as sns  #依赖pandas
...
def Name(filename, fytime, plattype, outputpath, outTXTpath, prjType):
    if (prjType == 'NOM'):
        prj = "MAPN"
    elif (prjType == "GLL"):
        prj = "MAPG"
    ...
    if (len(shortName) == 8):
        ...
        iquamname = "%s%s_QCS_IQUAM_%s.png" % (outputpath, filename, prj)
        scattertitle = "FY4A_IQUAM_SST %s" % (fytime.__format__('%Y-%m-%d %H:%M:%S'))
        histtitle = "FY4A_IQUAM_SST %s" % (fytime.__format__('%Y-%m-%d %H:%M:%S'))
        iquamtitle = "IQUAM_SST_%s %s-01 000000--%s" % (prj,
                                                        fytime.__format__('%Y-%m'),
                                                        fytime.__format__('%Y-%m-%d %H%M%S'))  # 标题
        # 带标签中间文件
        txtnamelabel = "%s%s00000000%s_QCS_IQUAM(FULL)_DATA_BIAS.txt" % (outTXTpath, filename[:50], filename[73:])
        txtname = "%s%s00000000%s_QCS_IQUAM_DATA_BIAS.txt" % (outTXTpath, filename[:50], filename[73:])  # 全类型中间文件
    else:
        ...
        iquamname = "%s%s_QCS_IQUAM-%s_%s.png" % (outputpath, filename, platshortStr, prj)
        scattertitle = "FY4A_IQUAM(%s)_SST %s" % (platshortStr, fytime.__format__('%Y-%m-%d %H:%M:%S'))
        histtitle = "FY4A_IQUAM(%s)_SST %s" % (platshortStr, fytime.__format__('%Y-%m-%d %H:%M:%S'))
        iquamtitle = "IQUAM(%s)_SST_%s %s-01 000000--%s" % (
            platshortStr, prj, fytime.__format__('%Y-%m'), fytime.__format__('%Y-%m-%d %H%M%S'))  # 标题
        # 带标签中间文件
        txtnamelabel = "%s%s00000000%s_QCS_IQUAM-%s_DATA_BIAS.txt" % (
            outTXTpath, filename[:50], filename[73:], platshortStr)
        txtname = "%s%s00000000%s_QCS_IQUAM_DATA_BIAS.txt" % (outTXTpath, filename[:50], filename[73:])  # 全类型中间文件
...
def Interp(tlat, tlon, slat, slon, sdata, dqf, spaceThreshold):
    """
    空间插值
    :param tlat: 目标纬度
    :param tlon: 目标精度
    ...
    slat[np.where(slat == -999.)] = np.nan
    slon[np.where(slon == -999.)] = np.nan
    print slat
    # 2.转换经纬度到笛卡尔坐标系
...
def SpaceMatch(fy4, fy4_dqf, exterMatched, platformtype):
...
def WriteToTXT(mdat, fytime, filepath):
    """
    输出为TXT文件
