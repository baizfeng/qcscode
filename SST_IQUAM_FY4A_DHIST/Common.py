...
def Name(filename, fytime, plattype, outputpath, dataSour, isna):
    if (isna == 'false'):
        ...
        if (len(shortName) == 8):
            ...
            histname = "%s%s_QCS_IQUAM-%s_HIST_BIAS.png" % (outputpath, filename, platshortStr)
            scattername = "%s%s_QCS_IQUAM-%s_SCAT_ORIG.png" % (outputpath, filename, platshortStr)
            fy4name = "%s%s_QCS_MAPN_ORIG.png" % (outputpath, filename)
            ...
            iquamname = "%s%s_QCS_IQUAM-%s_MAPG_ORIG.png" % (outputpath, filename, platshortStr)
    elif (isna == 'true'):
        shortName = Plattypeprocess(plattype)
        if (len(shortName) == 8):
            ...
            fy4title = "%s_SST_MAPN %s" % (dataSour, fytime.__format__('%Y-%m-%d'))  # 标题
            iquamtitle = "%s_IQUAM(%s)_SST %s" % (dataSour, platshortStr, fytime.__format__('%Y-%m-%d'))
            iquamname = "%s%s_QCS_IQUAM-%s_MAPG_ORIG_NA.png" % (outputpath, filename, platshortStr)
...
def ReadTXTData(filepath, fy4time):
...
def Read_Data(filepath):
    ...
    with Dataset(filepath, 'r') as fid:
        fy4_data = fid.variables['SST Daily Mean Product'][:]

    if (isinstance(fy4_data, ma.MaskedArray)):
        fy4_data = fy4_data.data
    fy4_data[(fy4_data > 45) | (fy4_data < -5)] = -999
...
def DrawScatter(mdat, param, totalNum, names):
    ...
    ax.set_xlabel('FY4A SST' + '(' + u'\u2103' + ')', fontsize=12)
